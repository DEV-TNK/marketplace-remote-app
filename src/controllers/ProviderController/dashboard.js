...
const getlastFourPaidJobs = async (req, res) => {
  try {
    ...
    // Retrieve the job details from the Job model only
    const jobsFromJobModel = await Job.find({
      _id: { $in: jobIds },
    }).sort({ createdAt: -1 });

    // Sort the results by createdAt and limit to the last 4
    const sortedJobs = jobsFromJobModel.sort((a, b) => b.createdAt - a.createdAt).slice(0, 4);

    // Format the response for regular jobs
    const formattedJobs = sortedJobs.map((job) => {
      return {
        _id: job._id,
        jobTitles: [job.jobTitle],
        ...
        type: "Single Job",
        currency: job.currency,
        number: 1, // Assuming single jobs have only one person
      };
    });
