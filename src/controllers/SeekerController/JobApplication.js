...
const totalJobsApplied = async (req, res) => {
  try {
    ...
    const JobsApplied = await Applicant.countDocuments({ userId });
...
const acceptOrRejectOffer = async (req, res) => {
  try {
    ...
    if (status === "true") {
      ...
      return res.status(200).json({ message: "Offer Accepted Successfully", offerDetails, jobDetails });
    } else {
      ...
      return res.status(200).json({ message: "Offer Rejected Successfully", offerDetails, jobDetails });
    }
  } catch (error) {
    console.log(error)
  }
...
const getApplicationsBySeeker = async (req, res) => {
  try {
    ...
    const applicationsPerMonth = applications.reduce((acc, application) => {
      ...
      const monthName = applicationDate.toLocaleString('default', { month: 'long' }); // Get full month name
